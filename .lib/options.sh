...
getoptions() {
	...
	_setup() {
		[ "${1#-}" ] && _rest=$1
		_minargs=0 # Default minimum required arguments
		while loop "$@" && shift; do
			case $1 in
			minargs=*) _minargs=${1#minargs=} ;;
			*) kv "$1" _ ;;
			esac
		done
	}
	...
	_2 "if [ \$(eval \"echo \\\"\${$_rest}\\\" | wc -w\") -lt 1 ]; then" # Change $_minargs to 1
	_3 '\''echo "Error: At least 1 argument(s) required" >&2'\''               # Update error message
